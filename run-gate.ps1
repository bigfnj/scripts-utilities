...
$suites = @(
    @{ Name = 'smoke'
       ...
       What = 'toolbox, PATH, forensics sensors + the core, installer, triage and render suites' }
)
...
foreach ($s in $suites) {
    ...
    foreach ($line in ($out -split "`r?`n" | Where-Object { $_ -match '(core|installer|triage|render) suite: \d+ passed' })) {
